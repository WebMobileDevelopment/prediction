                        ...
                        <div class="card-body">
                            ...
                            <form action="{{ route('menu.update') }}" method="POST">
                                                ...
                                                <select class="form-control" name="menu" id="menu">
                                                    @foreach ($menulist as $menu1)
                                                        @if ($menu1->id == $menuElement->menu_id)
                                                            <option value="{{ $menu1->id }}" selected>{{ $menu1->name }}
                                                            </option>
                                                        @else
                                                    ...
                                                    @foreach ($roles as $role)
                                                        <tr>
                                                            <td>
                                                                ...
                                                                if ($temp === true) {
                                                                echo '<input checked type="checkbox" name="role[]"
                                                                    value="' . $role . '" class="form-control" />';
                                                                } else {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="text" class="form-control" name="name"
                                                    value="{{ $menuElement->name }}" placeholder="Name" />
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>
                                ...
                                <a class="btn btn-primary"
                                    href="{{ route('menu.index', ['menu' => $menuElement->menu_id]) }}">Return</a>
                            </form>
                        </div>
                    </div>
                </div>
...
@section('javascript')
    <script src="{{ asset('render/js/axios.min.js') }}"></script>
    <script src="{{ asset('render/js/menu-edit.js') }}"></script>
